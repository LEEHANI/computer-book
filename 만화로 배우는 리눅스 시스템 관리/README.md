...
- [cut](#17-로그-파일에서-필요한-줄만-뽑고-싶어cut)
- [sort, uniq](#18-같은-내용의-줄을-세어보고-싶어sort와-uniq)
- [sort, >, >>](#19-csv-파일을-열의-내용에-따라-정렬하고-싶어sort와-리다이렉트)
...
## 17. 로그 파일에서 필요한 줄만 뽑고 싶어(cut)  
- 문자열 가공 명령어 
  + ```tail```, ```grep```, ```sed```
- 문자열은 파이프라인(```|```)을 이용해 조합, 명령어 치환 가능 
- ```cut```은 파이프로 넘어온 내용의 각 줄마다 필요한 부분만 잘라내서 돌려주는 명령어  
  + ```192.168.11.3 - - [8/Feb/2013:15:02:57] +0900] "GET /path/to/page HTTP/1.1" 200``` 
  + ex) ```cut -d " " -f 7``` //구분자는 " "
    * -d(delimiter) 구분자
    * -f(fields) 추출할 위치
  + 결과 : ```/path/to/page```
...
## 18. 같은 내용의 줄을 세어보고 싶어(sort와 uniq)
- ```sort``` 내용을 알파벳 순서로 재정렬 
  + ```cat input.txt | sort ```
  + -r 은 내림차순 정렬
- ```uniq``` 중복 제거. 이어진 목록만 제거하므로 sort를 선행하는 것이 좋다. 
  + ```cat input.txt | uniq ```  
  + -c 각각 명령이 몇 번 등장했는가 세줌 
  + ```cat input.txt | sort | uniq -c | cat```
- ```head```, ```tail``` 로 10줄만 추출 가능 
  + -n 으로 출력하는 줄 수 변경   
```  
#!/bin/bash
...
## 19. CSV 파일을 열의 내용에 따라 정렬하고 싶어(sort와 리다이렉트)
- ```cut```의 -f 옵션으로 여러 숫자를 지정하거나 열 범위 지정 가능 
  + ```cut -d "," -f 1,3,4```
  + ```cut -d "," -f 1-4```
- ```sort```의 -t 옵션으로 구분자를 지정하고 -k 옵션으로 정렬하고 싶은 열 번호 지정 
  + ```sort -t "," -k 3``` 단순 정렬 
    * 단순 정렬된다는 말은 [1, 2, 11, 10]일 때, 첫자리 숫자만 보고 정렬하고 [1, 11, 10, 2] 그 다음자릿수로 정렬 [1, 10, 11, 2] 하는 방식 
  + ```sort -t "," -k 3 -n ``` 숫자 정렬 
    * -n 옵션을 주면 숫자 정렬 가능
    * 만약 숫자들이 오른쪽 정렬 되어 있다면 [' 1', ' 2', '11', '10'] -n 옵션을 주지 않아도 잘 정렬됨 (숫자이지만 편의상 따옴표 추가)
- ```>``` 리다이렉트를 사용하면 내용을 텍스트 파일로 출력해준다
  + ```>```가 하나라면, 이미 파일이 존재하면 지우고 새로운 파일을 만든다. 만약 이미 파일이 존재하면 덮어씀
  + ```>>```가 두개라면, 기존 파일에 추가    
<br/>
