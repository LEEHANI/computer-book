...
2. CA의 비밀키로 서버의 공개키를 암호화한 인증서를 서버에게 돌려준다. 
3.  
웹 서버는 클라이언트의 요청이 들어왔을 때, ‘CA의 개인키’로 암호화된 인증서를 전달합니다.
클라이언트의 브라우저에는 신뢰할 수 있는 기관의 공개키가 저장되어있습니다. 해당하는 CA의 공개키를 찾아 인증서를 복호화하면, 웹 서버의 ‘공개키’를 얻을 수 있습니다.
클라이언트는 대칭키를 생성할 수 있는 난수를 ‘웹 서버의 공개키’로 암호화하여 전송합니다.
서버와 클라이언트는 각자 난수를 가지고 대칭키를 생성합니다.
